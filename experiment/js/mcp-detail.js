...
const SCENARIO_DESCRIPTIONS = {
  A: 'Imagine your manager has sent you an email with several time slots when he is available for a meeting. You want to check your own calendar to find a time that works for both of you and then schedule a meeting with your manager. This task requires you to use the <strong>Google Calendar MCP</strong> server to check your calendar availability and schedule the meeting.',
  B: 'Imagine you had a medical appointment yesterday and want to follow up on the instructions provided during the visit. You want to find the medical file created or updated yesterday on your computer and review its contents to determine what you need to do next. This task requires you to use the <strong>Filesystem MCP</strong> server to search for the relevant file and review its contents.',
  C: 'Imagine you recently had a medical appointment and want to follow up on some details. Using your calendar, you want to find the appointment and note the date and provider. Then, you want to locate the corresponding medical file on your computer (e.g., a visit summary, test results, or notes) and review its contents to check the diagnosis and any prescribed medications. This task requires both the <strong>Google Calendar MCP</strong> and <strong>Filesystem MCP</strong> servers, as shown below.',
};
...
function render() {
  ...
  container.innerHTML = `
    <div class="hp-scenario-desc" style="font-size:15px;"><strong>${PROMPT_LABELS[prompt]}:</strong> ${SCENARIO_DESCRIPTIONS[prompt]}</div>`;
