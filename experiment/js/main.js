...
function trackScenarioComplete(prompt, mcp) {
  ...
  const startedAt = data.currentScenarioStartedAt || null;
  const now = new Date().toISOString();
  const durationSec = startedAt ? parseFloat(((new Date(now).getTime() - new Date(startedAt).getTime()) / 1000).toFixed(1)) : null;
  ...
  data.currentScenarioStartedAt = null; // clear after use
