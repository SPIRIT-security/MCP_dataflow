...
function renderPage() {
  ...
  // Render blocks (text + tools interleaved) or legacy tools
  if (content.blocks) {
    renderBlocks(content.blocks);
  } else if (content.tools) {
    // Legacy: convert to blocks
    const blocks = content.tools.map(t => ({ type: 'tool', ...t }));
    if (content.finalResponse) {
      blocks.push({ type: 'text', html: content.finalResponse });
    }
    renderBlocks(blocks);
  }
