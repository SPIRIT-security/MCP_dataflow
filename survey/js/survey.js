...
function collectAnswers() {
  ...
  // Q2: single-choice radio; Q3-Q28: Likert
  for (let i = 2; i <= 28; i++) { const el = form.querySelector(`input[name="q${i}"]:checked`); responses[`q${i}`] = el ? el.value : null; }
  // Q29-Q31: Future Intention (Likert)
  for (let i = 29; i <= 31; i++) { const el = form.querySelector(`input[name="q${i}"]:checked`); responses[`q${i}`] = el ? el.value : null; }
  // Q32-Q35: Privacy Concern (A/B/C/D target)
  for (let i = 32; i <= 35; i++) { const el = form.querySelector(`input[name="q${i}_target"]:checked`); responses[`q${i}_target`] = el ? el.value : null; }
  return responses;
}
...
function validateAll() {
  ...
  for (let i = 29; i <= 31; i++) { if (!r[`q${i}`]) unanswered.push(`Q${i}`); }
  for (let i = 32; i <= 35; i++) { if (!r[`q${i}_target`]) unanswered.push(`Q${i}`); }
  return unanswered;
}
